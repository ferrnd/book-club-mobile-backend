...
export const criar = async (req, res) => {
  try {
    ...
    if (!caracteristicas_pt) {
      return res
        .status(400)
        .json({ error: 'O campo "caracteristicas_pt" é obrigatório!' });
    }
    if (!caracteristicas_en) {
      return res
        .status(400)
        .json({ error: 'O campo "caracteristicas_en" é obrigatório!' });
    }
    if (!descricao_pt) {
      return res
        .status(400)
        .json({ error: 'O campo "descricao_pt" é obrigatório!' });
    }
    if (!descricao_en) {
      return res
        .status(400)
    ...
    return res
      .status(201)
      .json({ message: "personagem criado com sucesso!", data });
  } catch (error) {
...
export const buscarTodos = async (req, res) => {
  try {
    ...
    if (!registros || registros.length === 0) {
      return res
        .status(400)
        .json({ message: "Nenhum registro de personagem encontrado." });
    }
...
export const buscarPorId = async (req, res) => {
  try {
    ...
    if (isNaN(id)) {
      return res
        .status(400)
        .json({ error: "O ID do personagem não é um número válido." });
    }
...
export const atualizar = async (req, res) => {
  try {
    ...
    if (!personagem) {
      return res
        .status(404)
        .json({ error: "personagem não encontrado para atualizar." });
    }
...
export const deletar = async (req, res) => {
  try {
    ...
    if (!personagem) {
      return res
        .status(404)
        .json({ error: "personagem não encontrado para deletar." });
    }
