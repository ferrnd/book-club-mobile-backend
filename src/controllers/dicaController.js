...
export const criar = async (req, res) => {
    try {
        ...
        if (!tipo_pt){
            return res.status(400).json({ error: 'O campo "tipo em português" é obrigatório!' });
        }
        if (!tipo_en){
            return res.status(400).json({ error: 'O campo "tipo em inglês" é obrigatório!' });
        }
        if (!conteudo_pt) {
            return res.status(400).json({ error: 'O campo "conteudo em português" é obrigatório!' });
        }
        if (!conteudo_en) {
            return res.status(400).json({ error: 'O campo "conteudo em inglês" é obrigatório!' });
        }
...
export const atualizar = async (req, res) => {
    try {
        ...
        return res.status(200).json({ message: `O registro "${data.tipo_pt}" foi atualizado com sucesso!`, data });
    } catch (error) {
...
export const deletar = async (req, res) => {
    try {
        ...
        return res.status(200).json({ message: `O registro "${dica.nome}" foi deletado com sucesso!`, deletado: dica });
    } catch (error) {
